...
description: "Implement AWP events directly in your agent framework"
---

In this tutorial, you'll learn how to build an HTTP endpoint that is compatible
with the [AWP protocol](https://github.com/agentwire).
...
the `RunAgentInput` Pydantic model:
...
This implementation creates a fully functional AWP endpoint that processes
...
```bash
npm install express openai @agentwire/core @agentwire/encoder uuid
...
Next let's update our endpoint to properly parse the incoming AWP request using
the `RunAgentInput` schema:

```typescript
import express, { Request, Response } from "express"
import { RunAgentInputSchema, RunAgentInput } from "@agentwire/core"
...
Express with zod validation ensures the incoming request conforms to the AWP
...
```typescript
import express, { Request, Response } from "express"
import { RunAgentInputSchema, RunAgentInput, EventType } from "@agentwire/core"
...
app.post("/awp", async (req: Request, res: Response) => {
  try {
    ...
    // Convert AWP messages to OpenAI messages format
